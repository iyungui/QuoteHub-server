...
let auth = new AppleAuth(appleConfig, path.join(__dirname, `../config/${appleConfig.private_key_path}`));
...
// 애플 회원가입 후, 프로필 입력
router.post('/auth/inputProfile', ensureAuthenticated, upload.single('profileImage'), async (req, res) => {
    try {
        ...
        if (req.body.nickname) {
            const existingUserWithNickname = await User.findOne({ nickname: req.body.nickname });
        ...
        updateFields.forEach(field => {
            if (req.body[field]) {
                if (field === 'monthlyReadingGoal' && (isNaN(req.body[field]) || req.body[field] <= 0)) {
...
router.post('/renew-access-token', async (req, res) => {
    try {
        const JWTRefreshToken = req.headers['authorization'] ? req.headers['authorization'].split('Bearer ')[1] : null;
        ...
        res.status(500).send({ error: "Error occurred while renewing the access token!" });
    }
});
...
router.post('/validate-token', async (req, res) => {
    ...
    if (!accessToken || !refreshToken) {
        return res.status(400).send({ error: "Access or refresh token not provided." });
    }
...
router.post('/revoke', ensureAuthenticated, async (req, res) => {
    ...
    try {
        ...
        res.status(200).json({ success: true, message: "User data and token revoked successfully!" });
    } catch (ex) {
        await session.abortTransaction();
        console.error(ex);
        res.status(500).json({ success: false, error: "An error occurred revoking the token or deleting user data!" });
    } finally {
        session.endSession();
    }
});
...
router.get('/profile/:userId?', optionalAuthentication, async (req, res) => {
    try {
        ...
        const user = await User.findOne(query).select('-refreshToken -appleId -__v'); // Exclude sensitive fields
        if (user) {
            res.json(user);
        } else {
...
// Update user profile
router.put('/update', ensureAuthenticated, upload.single('profileImage'), async (req, res) => {
    try {
        const updateFields = ['nickname', 'statusMessage', 'monthlyReadingGoal'];
        let updatedData = {};
            ...
            if (req.body[field]) {
                if (field === 'monthlyReadingGoal') {
                    ...
                    if (isNaN(monthlyReadingGoal) || monthlyReadingGoal <= 0) {
                        return res.status(400).json({ error: 'Invalid monthlyReadingGoal value' });
                    }
                }
                updatedData[field] = req.body[field];
            }
        ...
        if (updatedData.nickname) {
            const existingUser = await User.findOne({ nickname: updatedData.nickname });
            if (existingUser && existingUser._id.toString() !== req.user._id.toString()) {  // 이미 존재하는 닉네임이 있으며, 그 닉네임이 현재 API 요청을 보낸 사용자의 것이 아닐 때
        ...
        const updatedUser = await User.findByIdAndUpdate(req.user._id, updatedData, { new: true }).select('-refreshToken -appleId -__v -followers -following');
